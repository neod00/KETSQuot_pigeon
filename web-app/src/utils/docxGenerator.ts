...
// K-ETS 명세서 검증 계약서용 DOCX 생성
export const generateKetsDocx = async (data: any, companyName: string = "기업명") => {
    if (typeof window === 'undefined') {
    ...
    try {
        ...
        const templatePath = "/templates/K-ETSemission_template.docx";
        ...
        PizZipUtils.getBinaryContent(templatePath, (error: Error | null, content: string) => {
            ...
            // 파일 이름 생성 (LRQA_온실가스 명세서 검증 제안서 계약서_회사명.docx)
            const fileName = `LRQA_온실가스 명세서 검증 제안서 계약서_${companyName || "기업명"}.docx`.replace(/[/\\?%*:|"<>]/g, '-');
            saveAs(out, fileName);
        });
    } catch (e) {
