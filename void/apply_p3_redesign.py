...
# 1. Define the New Styles for Page 3
new_styles = """
...
#pf3 { height: auto !important; min-height: 1122px !important; }
#pf3 * { position: static !important; }
"""

# Inject styles into the first <style> block
content = content.replace('</style>', new_styles + '</style>', 1)

# 2. Define the New HTML Structure for Page 3
new_p3_html = """
...
# Replace the original pf3 block
# Use regex to find <div id="pf3" ...> ... </div>
# Taking care of nested divs if any (though pf divs are usually flat)
pattern = r'<div id="pf3".*?</div>\s*</div>' # In original, there's a div wrapper for page content
# Actually let's just find id="pf3" and find its closing tag
start_tag = content.find('<div id="pf3"')
if start_tag != -1:
    # Find the next page pf4 to find the boundaries
    end_boundary = content.find('<div id="pf4"')
    if end_boundary != -1:
        # Replacement
        content = content[:start_tag] + new_p3_html + content[end_boundary:]
        print("Successfully replaced Page 3 HTML")
    else:
